...
class Post(ABC):
    ...
    def like(self, liker): # like a post
        if self.user.get_online() and liker not in self.likes:
            ...
            if self.user.get_name() != liker.get_name():  # check if the liker is not the user
                self.user.notify(self, "like", liker)

    def comment(self, commenter, content):  # comment a post
        if self.user.get_online() and self.user.get_name() != commenter.get_name():
